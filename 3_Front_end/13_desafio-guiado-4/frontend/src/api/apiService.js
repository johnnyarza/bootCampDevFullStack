...
const API_URL = 'http://localhost:3003/grade/';
...
async function getAllGrades() {
  ...
  //neste caso faz-se a verificação se alguma nota foi excluida da API,
  //uma vez que quando exclui se a nota ela é apagada do banco de dados,
  //contudo ela continuará aparecendo no frontend. Logo, as notas
  //que nao existem na API, mas existem em allCombinations serão adicionadas
  //no frontend com o valur 0 e com a atributo isDeleted: true
  let maxId = -1;
  grades.forEach(({ id }) => {
    if (id > maxId) {
      maxId = id;
